...
var val = ''
const $ = window.$;
export class PremiumPaymentLoadDetails extends React.Component {
    ...
    getData = () => {
        ...
        let query = `{            
            Payment820FileDetails(State:"${this.state.State ? this.state.State : ''}",StartDt: "${startDate}",EndDt: "${endDate}",FileID:"${this.state.incoming_fileId}" ,RecType:"", EFTCHK:"${this.state.EFTCHK}") {
                RecCount
                Sender
                Receiver_Name
                Payer_Name
        ...
        fetch(Urls._Payment820, {
            ...
            .then(res => {
                this.setState({
                    rowData: res.data.Payment820FileDetails,
    ...
    getTransactions = (fileId) => {
        ...
        let query = `{            
       Payment820DetailWise  (FileID : "` + fileId + `") {
        FileID
        FileName
        FileDate
        LastName
        FirstName
        MiddleName
        IndivisualIdentifiler
        RemittanceReferenceNo
        DetailedPremiumPaymentAmt
        BilledPremiumAmt
        CurrentAdjustmentAmt
        CurentAdjustmentReasonCode
        ReportDateRange
            }
        }`
        if (Strings.isDev) { process.env.NODE_ENV == 'development' && console.log(query) }
        fetch(Urls._Payment820, {
            ...
            .then(res => {
                this.setState({
                    claims_rowData: res.data.Payment820DetailWise,
                    Ag_grid_FileName: res.data.Payment820DetailWise[0].FileName,
                    Ag_grid_fileDate: res.data.Payment820DetailWise[0].FileDate
    ...
    _renderList = () => {
        let columnDefs = [
            { headerName: "File Name", field: "FileName", width: 150, cellStyle: { wordBreak: 'break-all', 'white-space': 'normal', color: '#139DC9', cursor: 'pointer' }, pinned: 'left' },
            { headerName: "File Date", field: "FileDate", width: 100, pinned: 'left'},
            ...
            { headerName: "Remittance Amount", field: "RemittanceAmount", width: 120, },
            ...
            { headerName: "Total", field: "TotalClaim",  width: 120 },
    ...
    _renderClaims() {
        ...
        let columnDefs = [
            ...
            { headerName: "Individual Identifiler", field: "IndivisualIdentifiler", width: 150, cellStyle: { wordBreak: 'break-all', 'white-space': 'normal' } , pinned: 'left'},
            { headerName: "Last Name", field: "LastName", width: 150, cellStyle: { wordBreak: 'break-all', 'white-space': 'normal' }, pinned: 'left' },
            { headerName: "First Name", field: "FirstName", width: 150, cellStyle: { wordBreak: 'break-all', 'white-space': 'normal' }, pinned: 'left' },
            // { headerName: "Middle Name", field: "MiddleName", width: 150, cellStyle: { wordBreak: 'break-all', 'white-space': 'normal' }, pinned: 'left' },
            { headerName: "Remittance Reference No ", field: "RemittanceReferenceNo", width: 150, cellStyle: { wordBreak: 'break-all', 'white-space': 'normal' } },
            { headerName: "Report Date", field: "ReportDateRange", width: 150, cellStyle: { wordBreak: 'break-all', 'white-space': 'normal' } },
            { headerName: "Detailed Premium Payment Amt", field: "DetailedPremiumPaymentAmt", width: 150, cellStyle: { wordBreak: 'break-all', 'white-space': 'normal' } },

            { headerName: "Billed Premium Amt", field: "BilledPremiumAmt", width: 150, cellStyle: { wordBreak: 'break-all', 'white-space': 'normal' } },
            { headerName: "Current Adjustment Amt ", field: "CurrentAdjustmentAmt", width: 150, cellStyle: { wordBreak: 'break-all', 'white-space': 'normal' } },

            { headerName: "Current Adjustment Reason Code", field: "CurentAdjustmentReasonCode", width: 180, cellStyle: { wordBreak: 'break-all', 'white-space': 'normal' } },
        ]
        ...
        return (
            <div>
                ...
                <div className="ag-theme-balham" style={{ padding: '0', marginTop: '24px' }}>
                    <h6 className="font-size">Premium Payment Information For <label style={{ color: 'var(--main-bg-color)' }}>(File Name:-{this.state.Ag_grid_FileName} , File Date:-{this.state.Ag_grid_fileDate})</label></h6>
                    <AgGridReact
                        modules={this.state.modules}
                        columnDefs={columnDefs}
                        autoGroupColumnDef={this.state.autoGroupColumnDef}
    ...
    render() {
        ...
        return (
            <div>
                <h5 className="headerText">Premium Payment Load Details {this.state.subtitle ? <label style={{ fontSize: "14px" }}>({this.state.subtitle})</label> : ""}</h5>
                {/* {this.renderFilters()} */}
                {this._renderTopbar()}
                {
                    this.state.gridType
