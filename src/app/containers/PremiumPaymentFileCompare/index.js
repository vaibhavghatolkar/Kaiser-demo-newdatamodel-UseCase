...
let val = ''
export class PremiumPaymentFileCompare extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            FileName: [],
            selected_FileID: props.location.state == undefined ? '' : (props.location.state && props.location.state.data && props.location.state.data[0].incoming_fileId == "" ? "" : props.location.state.data[0].incoming_fileId),
            ...
            summaryCount: [
                { name: 'Total Payment', value: 64567 },
                { name: 'Reconciled', value: 60000 },
                { name: 'Errors', value: 4567 },
            ],
            summaryCountNew: [
    ...
    _renderClaimTables = (array) => {
        ...
        array.forEach(item => {
            ...
            let flag = ''
            let MonthlyStatus = ""
            if (item.name == 'Active in QNXT - Not Paid' && item.flag == 1) {
                url = Strings.OutboundActive
            } else if (item.name == 'Term in QNXT but Paid' && item.flag == 1) {
                url = Strings.OutboundTerm
            } else if (item.name == 'Active in QNXT - Not Paid') {
                url = Strings.ActiveQnxt820
            } else if (item.name == 'Term in QNXT but Paid') {
                url = Strings.TermQnxt820
            } else if (item.name == 'RateCode Mismatch') {
                url = Strings.RateCode820Mismatch
            }
    ...
    renderClaimDetails = () => {
        ...
        let stage_1 = [
            ...
            { 'name': 'RateCode Mismatch', 'value': 1834, 'isClick': true },
            { 'name': 'Active in QNXT - Not Paid', 'value': 1278, 'isClick': true },
            // { 'name': 'Active - Over Paid', 'value': 1146 },
            // { 'name': 'Active - Under Paid', 'value': 750 },
            { 'name': 'Term in QNXT but Paid', 'value': 1455, 'isClick': true },

        ]
        let stage_6 = [
            { 'header': 'Types of Plan' },
            { 'name': 'MOLINA', 'value': 600 },
            { 'name': 'MOLCHIP', 'value': 750 },
            { 'name': 'MOL IMED', 'value': 1230 },
        ]

        let stage_2 = [
            { 'header': 'Type of Payment' },
            { 'name': 'Rx Payment', 'value': 1001 },
            { 'name': 'VSP Payment', 'value': 1267 },
            { 'name': 'Beacon Payment', 'value': 1389 },
        ]

        let stage_3 = [
            { 'header': 'Outbound Errors' },
            { 'name': 'Active in QNXT - Not Paid', 'value': 612, 'isClick': true, flag: 1 },
            { 'name': 'Term in QNXT but Paid', 'value': 1943, 'isClick': true, flag: 1 },
        ]
            ...
            <div>
                ...
                <div className="row">
                    <div className="col-12">
                        ...
                        <label className="general-header">Difference Between 820 And QNXT</label>
