...
var val = ''
export class Outbound_Claim_999_response_Kaiser extends React.Component {
    ...
    getTransactions() {
        ...
        query = `{
            Data999(RecType: "Kaiser", TrasactionType: "837Kaiser", FileId: "${fileId}", FileName: "", StartDt: "${startDate}", EndDt: "${endDate}", State: "${this.state.State}", page: ${this.state.page}, OrderBy: "${this.state.orderby}", GridType:${this.state.gridType}) {
              FileId
              FileName
              Date
              Submitter
          ...
          if(this.state.flag999 == 1){
            query = `{
                Data999(RecType: "Kaiser", TrasactionType: "837Kaiser", FileId: "${fileId}", FileName: "", StartDt: "${startDate}", EndDt: "${endDate}", State: "${this.state.State}", page: ${this.state.page}, OrderBy: "${this.state.orderby}", GridType:${this.state.gridType}, Type:"") {
                  FileId
                  FileName
                  Date
                  Submitter
    ...
    render999Details(refId) {
        let transactionType = this.state.flag999 == 1 ? '837' : '837Encounters'
            ...
            let   query = `{
                Data999_ResponseKaiser  (RefId:${refId}) {
                    FileId
                    RefId
                    Response
                }
        ...
        fetch(this.state.flag999 == 1 ? Urls._inbound_common_data : Urls._common_data, {
            ...
            .then(res => {
                if (res.data) {
                    this.setState({
                        Response: res.data.Data999_ResponseKaiser[0].Response,
                        showDetails: true
                    })
                }
            })
