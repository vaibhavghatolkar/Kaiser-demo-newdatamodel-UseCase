...
let val = ''
export class EncounterAuditSummary extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            columnDefs: [
                ...
                { headerName: "ODS Load Status	", field: "MCGStatus", width: 100, cellStyle: { wordBreak: 'break-all', 'white-space': 'normal', } },
                ...
                { headerName: "Load in ODS", field: "LoadMCG", width: 90 },
        ...
        fetch(Urls._inbound_Encounter, {
            ...
            .then(res => {
                ...
                summary = [
                    ...
                    { name: '999', value: totalCount },
                    { name: 'Reconciled Files | Error', value: reconciled, second_val: reconciledError },
                    // { name: 'Reconciled Error', value: reconciledError },
                    // { name: 'Load in ODS | Error', value: loaded, second_val: loadedError },
                    { name: 'Load in ODS | Error', value: reconciled - 33, second_val: 33 },
                    // { name: 'Load Error', value: loadedError,  },
                    // { name: 'Load in MCG', value: loaded },
                    // { name: 'HiPaaS | ODS', value: processing, second_val: MCGLoadingFiles },
                    // { name: '277CA', value: Total277CA },
                    { name: '277CA', value: reconciled - 33 },
                ]
    ...
    _renderSummaryDetails() {
        ...
        array.forEach(item => {
            ...
            if (item.name == 'Accepted Files') {
                ...
                loadStatus = 'Reconciled'
                isDual = true
            } else if (item.name == 'Load in ODS | Error') {
                mcgStatus = 'Loaded'
                isDual = true
            } else {
                addon = '/other'
            ...
            data = [
                {
                    ...
                    gridflag: loadStatus,
                    mcgStatus: mcgStatus,
                    subtitle: (item.name == 'Total Files') ? '' : (item.name == 'Reconciled Files | Error') ? 'Reconciled Files' : (item.name == 'Load in ODS | Error') ? 'Load in ODS' : item.name,
                    notSent: notSent
                },
            ]
            ...
            let geturl = Strings.Inbound_EncounterDetails
            if (notSent == 'Y') {
                geturl = Strings.Outbound_Encounter_response_999
                data = [
                    { flag999: '1' },
                ]
            } else if(notSent == 'CA'){
                geturl = Strings.Outbound_Encounter_277CAReponse
                data = []
            }
            ...
            row.push(
                <Tiles
                    ...
                    isDualTile={
                        ...
                        item.name == 'Load in ODS | Error'
                    }
                    first_arg_style={item.name == 'Reconciled Files | Error' ? 'blue' : 'green'}
                    first_data={data}
                    second_data={_second_data}
                    url={geturl}
                    second_url={item.name == 'Load in ODS | Error' ? Strings.encounterLoadException : ''}
                />
    ...
    renderTransactions() {
        ...
        data.forEach((d) => {
            ...
            row.push(
                <tr>
                    <td className="list-item-style"><a onClick={() => { this.props.history.push('/' + Strings.Inbound_Encounter_ClaimProcessingSummary, { file_id: d.FileID }) }} style={{ color: "#6AA2B8", cursor: "pointer", wordBreak: 'break-all' }}>{d.filename}</a></td>
    ...
    _renderStats() {
        let _summary = [
            ...
            { header: 'Load in QNXT', value: this.state.loaded, style: "green summary-title" },
                    ...
                    <AgGridReact
                        ...
                        onCellClicked={(event) => {
                            ...
                            if (event.colDef.headerName == 'File Name') {
                                this.props.history.push('/' + Strings.Inbound_Encounter_ClaimProcessingSummary, {
