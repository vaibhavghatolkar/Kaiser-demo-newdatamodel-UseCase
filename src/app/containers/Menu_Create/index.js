...
export class MenuCreate extends React.Component {
    ...
    componentDidMount() {
        ...
        this.getData(this.setState.fileId)
        this.getbinduser()
    }
    AddUserRole() {
        var query = `mutation{
    ...
    getData(fileId) {
        ...
        let query = '{UserwiseMenu (role_id:' + fileId + `) {
            role_id
            menu_id
            menu_description
            sequence_id
    ...
    ChangeVal(event, key) {
        ...
        this.setState.fileId = event.target.options[event.target.selectedIndex].value;
    ...
    Update() {
        if (this.setState.fileId != 0) {
            ...
            var query = 'mutation{ updateuserwisemenu(roleid : ' + this.setState.fileId + ' ' +
                'uncheck :"' + str2 + '"' +
                'check :"' + str1 + '"' +
